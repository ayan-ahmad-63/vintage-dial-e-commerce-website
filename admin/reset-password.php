...
if (empty($token)) {
    ...
    try {
        $stmt = $db->prepare("SELECT * FROM admins WHERE reset_token = ? AND reset_token_expiry > NOW()");
        $stmt->execute([$token]);
        $admin = $stmt->fetch();
...
if ($isValidToken && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($password)) {
        ...
        try {
            ...
            $up = $db->prepare("UPDATE admins SET password = ?, reset_token = NULL, reset_token_expiry = NULL WHERE id = ?");
            $up->execute([$hashed, $admin['id']]);
...
<head>
  ...
  <title>Admin Reset Password | Vintage Dial</title>
...
<body>
  <div class="container">
    <div class="auth-box">
      <h3 class="text-center font-weight-bold mb-4" style="color:#111;">Admin Reset Password</h3>
      ...
      <?php if ($success): ?>
        <div class="alert alert-success text-center" style="font-size: 13px; border-radius: 8px;">
          ...
          <a href="login.php" class="btn btn-dark btn-sm font-weight-bold px-4 py-2 mt-3" style="border-radius:20px;">Admin Login</a>
        </div>
      <?php endif; ?>
