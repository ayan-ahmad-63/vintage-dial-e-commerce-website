...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email)) {
        ...
        try {
            $stmt = $db->prepare("SELECT id, full_name FROM customers WHERE email = ?");
            $stmt->execute([$email]);
            $user = $stmt->fetch();
            ...
            if ($user) {
                ...
                // Save token to DB
                $up = $db->prepare("UPDATE customers SET reset_token = ?, reset_token_expiry = ? WHERE id = ?");
                $up->execute([$token, $expiry, $user['id']]);
...
<head>
    ...
    <title>Forgot Password | Vintage Dial</title>
...
<body>
    <div class="container">
        <div class="auth-box">
            <h3 class="text-center font-weight-bold mb-3" style="color:#111;">Password Recovery</h3>
            <p class="text-muted text-center mb-4" style="font-size:13px;">Enter your email below and we will help you
            ...
            <div class="alert alert-success" style="font-size: 13px; border-radius: 8px;">
                ...
                <div class="mt-3 text-center">
                    <a href="<?= $resetLink ?>" class="btn btn-dark btn-sm font-weight-bold px-3"
                        style="border-radius:20px;">Reset Password Now</a>
                </div>
            </div>
            <?php endif; ?>
